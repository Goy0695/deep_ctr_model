...
class BaseNMF:
    ...
    def model_fn(features, labels, mode, params):
        ...
        # ------bulid weights------
        W_user = tf.get_variable(name='user_embedding', shape=[user_size, layers[0]],
                                 initializer=tf.glorot_normal_initializer())
        W_item = tf.get_variable(name='item_embedding', shape=[item_size, layers[0]],
                                 initializer=tf.glorot_normal_initializer())
        ...
        # ------build input for DNN----------
        user_embedding = tf.nn.embedding_lookup(W_user, user_ids)  # None*(k+1)
        item_embedding = tf.nn.embedding_lookup(W_item, item_ids)  # None*(k+1)
        deep_inputs = tf.concat([user_embedding, item_embedding], 1)
        ...
        with tf.variable_scope("Out"):
            deep_inputs = BaseNMF.add_layer(deep_inputs, 1, sub_name="out")
            # deep_inputs = tf.contrib.layers.fully_connected(inputs=deep_inputs, num_outputs=1,activation_fn=tf.identity,
            # weights_regularizer=tf.contrib.layers.l2_regularizer(l2_reg))
    ...
    def compile(self, model_dir=None):
        ...
        self.model = tf.estimator.Estimator(model_fn=BaseNMF.model_fn, model_dir=model_dir, params=mf_params,
                                            config=run_config)

    def train_and_evaluate(self, tr_files, va_files):
        train_spec = tf.estimator.TrainSpec(input_fn=lambda: BaseNMF.input_fn(tr_files,
                                                                              num_epochs=mf_params["num_epochs"],
                                                                              batch_size=mf_params["batch_size"]))

        eval_spec = tf.estimator.EvalSpec(input_fn=lambda: BaseNMF.input_fn(tr_files,
                                                                            num_epochs=1,
                                                                            batch_size=mf_params["batch_size"]))
    ...
    def train(self, tr_files, va_files):
        self.model.train(input_fn=lambda: BaseNMF.input_fn(tr_files,
                                                           num_epochs=mf_params["num_epochs"],
                                                           batch_size=mf_params["batch_size"]))

    def evaluate(self, va_files):
        self.model.evaluate(
            input_fn=lambda: BaseNMF.input_fn(va_files, num_epochs=1, batch_size=mf_params["batch_size"]))

    def predict(self, te_files, isSave=False, numToSave=10):
        P_G = self.model.predict(input_fn=lambda: BaseNMF.input_fn(te_files, num_epochs=1, batch_size=1),
                                 predict_keys="prob")
        if isSave:
