...
class NFM:
    ...
    def train_and_evaluate(self, tr_files, va_files):
        evaluator = tf.estimator.experimental.InMemoryEvaluatorHook(
            ...
            input_fn=lambda: NFM.input_fn(va_files, num_epochs=1, batch_size=nfm_params["batch_size"]),
            every_n_iter=nfm_params["val_itrs"])
        self.model.train(
            input_fn=lambda: NFM.input_fn(tr_files, num_epochs=nfm_params["num_epochs"],
                                          batch_size=nfm_params["batch_size"]),
            hooks=[evaluator])
